...
DebugTextManager.init = function (self, world, gui, is_server, network_event_delegate)
	...
	self._world = world
	self._gui = gui
	self._world_gui = World.create_world_gui(world, Matrix4x4.identity(), 1, 1)
	self._time = 0
...
DebugTextManager.output_world_text = function (self, text, text_size, position, time, category, color, viewport_name)
	...
	color = color or Vector3(255, 255, 255)
	local new_text = {
		id = Gui.text_3d(gui, text, material, text_size, font, tm, text_offset, 0, Color(color.x, color.y, color.z)),
		text = text,
		text_size = text_size,
		position = {
			x = position.x,
